...
public class Queues : MonoBehaviour
{
    ...
    public void Enqueue()
    {
        ...
        // add a new object
        objectQueue.Add(Instantiate(object_to_stack, pos_of_Top_Stack_item.position, Quaternion.identity)); // add object to stack
        ...
        objectQueue[objectQueue.Count - 1].name = randomIDGenerator; // new ID for object. 
        Debug.Log("Newly added objects name is " + object_to_stack.name);
        objectQueue[objectQueue.Count - 1].GetComponent<MeshRenderer>().material.color = new Color(
    ...
    public void PeekObject()
    {
        ...
        Debug.Log("The object you are looking at ID's " + objectQueue[0].name); // name of object that is at top of stack. 
        Debug.Log("Size of stack is " + objectQueue.Count); // state size of the stack after peeking just for verification 
    }

    public void Dequeue()
    {
        objectQueue[0].SetActive(false);
        Destroy(objectQueue[0]); // remove from top pos position. 
